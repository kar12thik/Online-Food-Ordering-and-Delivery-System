...
function filterRest(filterItem) {
  let restChecked = document.querySelectorAll(
    ".filter-list input.check:checked"
  );
  findCategory(restChecked);

  function findCategory(categoriesChecked) {
    const category_list = [];
    const category_reduce = Array.from(categoriesChecked).reduce(function (
      sum,
      input
    ) {
  ...
  return (
    <div className="border" data-testid={dataTestId}>
      <div>
        ...
        <div
          className={`${categorybar ? "block" : "hidden"} lg:block md:block`}
        >
          <div className="">
                ...
                return (
                  <li className="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                    <div className="flex items-center pl-3">
                      <input
                        id={id}
                        onClick={() => filterRest(filterItem)}
                        type="checkbox"
                        value={Val}
                        className="check w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                        rel="chicken"
                      />
                      <label
